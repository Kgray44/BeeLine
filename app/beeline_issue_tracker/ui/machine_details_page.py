...
from beeline_issue_tracker.analytics.models import MachineRiskSummary, RISK_MEDIUM
...
from beeline_issue_tracker.ui.widgets import BrandHeader, HoneycombBackground, IssueListView, StatusBadge
...
class MachineDetailsPage(HoneycombBackground):
    ...
    def _add_predictive(self, risk: MachineRiskSummary | None) -> None:
        ...
        if risk is None:
            ...
            empty.setObjectName("predictiveEmptyState")
        ...
        top = QHBoxLayout()
        top.setSpacing(12)
        top.addWidget(_risk_score_card(risk, self.theme_manager), 2)
        top.addWidget(_fact_card("Risk Level", _risk_level_label(risk.risk_level)), 1)
        top.addWidget(_fact_card("Confidence", risk.confidence), 1)
        layout.addLayout(top)

        guidance = QHBoxLayout()
        guidance.setSpacing(12)
        guidance.addWidget(_fact_card("Predicted Problem", risk.predicted_problem), 1)
        guidance.addWidget(_fact_card("Suggested Action", risk.suggested_action), 1)
        layout.addLayout(guidance)

        facts = (
            _fact_card("Last Issue Date", format_timestamp(risk.last_issue_at) if risk.last_issue_at else NO_DATA_TEXT),
            _fact_card("Recent Issues", str(risk.recent_issue_count)),
            _fact_card("Recurring Issues", str(risk.recurring_issue_count)),
            _fact_card("Average Time Open", _duration_minutes_text(risk.average_time_open_minutes)),
        )
        layout.addLayout(_card_grid(facts, columns=4))

        reasons_title = QLabel("Why this machine is risky")
        reasons_title.setObjectName("smallSectionTitle")
    ...
    def _add_issue_history(
        ...
        resolved_list = IssueListView("resolved", "Resolved Issues", "Search resolved issues...")
...
def _risk_score_card(risk: MachineRiskSummary, theme_manager: ThemeManager) -> QFrame:
    ...
    layout.setSpacing(8)
    label = QLabel("Risk Score")
    label.setObjectName("metricLabel")
    score = QLabel(f"{_risk_level_label(risk.risk_level)} | {risk.risk_score}")
    score.setObjectName("largeMetricValue")
    bar = RiskScoreBar(theme_manager)
    bar.set_score(risk.risk_score, _risk_level_label(risk.risk_level))
...
def _duration_minutes_text(minutes: int | None) -> str:
    if minutes is None:
        return NO_DATA_TEXT
