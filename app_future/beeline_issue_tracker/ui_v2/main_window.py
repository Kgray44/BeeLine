...
from beeline_issue_tracker.future_config_manager import save_machine_config as save_machine_config_file
...
from beeline_issue_tracker.ui_v2.priority_board import PriorityBoardPage, load_priority_board_snapshot
from beeline_issue_tracker.ui_v2.settings_page import SettingsPage
from beeline_issue_tracker.ui_v2.shift_handoff_page import ShiftHandoffPage, load_shift_handoff_snapshot
...
class PriorityBoardLoadTask(QRunnable):
    def __init__(self, request_id: int, repository: IssueRepository):
        super().__init__()
        self.request_id = request_id
        self.repository = repository
        self.signals = MachineLoadSignals()
    ...
    def run(self) -> None:
        try:
            snapshot = load_priority_board_snapshot(self.repository)
        except Exception as exc:
            logger.exception("Could not load priority board")
            _safe_emit(self.signals.failed, self.request_id, "priority_board", str(exc))
            return
        _safe_emit(self.signals.finished, self.request_id, snapshot)


class ShiftHandoffLoadTask(QRunnable):
    def __init__(self, request_id: int, repository: IssueRepository, window_key: str):
        super().__init__()
        self.request_id = request_id
        self.repository = repository
    ...
    def run(self) -> None:
        try:
            snapshot = load_shift_handoff_snapshot(self.repository, self.window_key)
        except Exception as exc:
            logger.exception("Could not load shift handoff")
            _safe_emit(self.signals.failed, self.request_id, "shift_handoff", str(exc))
            return
        _safe_emit(self.signals.finished, self.request_id, snapshot)


class IntakeSuggestionsTask(QRunnable):
    def __init__(self, request_id: int, repository: IssueRepository, machine_number: str, query: str):
        super().__init__()
        self.request_id = request_id
        self.repository = repository
        self.machine_number = machine_number
    ...
    def run(self) -> None:
        started_at = perf_now()
        try:
            suggestions = self.repository.find_intake_suggestions(
                machine_number=self.machine_number,
                query=self.query,
                limit=5,
            )
        except Exception as exc:
            logger.exception("Could not load intake suggestions")
            _safe_emit(self.signals.failed, self.request_id, "smart_intake", str(exc))
            return
        perf_log(
            "smart_intake.search",
        ...
        _safe_emit(self.signals.finished, self.request_id, {"query": self.query, "suggestions": suggestions})


class DataHealthTask(QRunnable):
    def __init__(self, request_id: int, repository: IssueRepository, paths: AppPaths):
        super().__init__()
        self.request_id = request_id
        self.repository = repository
        self.paths = paths
    ...
    def run(self) -> None:
        started_at = perf_now()
        try:
            summary = self.repository.build_data_health_summary(self.paths)
        except Exception as exc:
            logger.exception("Could not load data health")
            _safe_emit(self.signals.failed, self.request_id, "data_health", str(exc))
            return
        perf_log("data_health.load", elapsed_ms=elapsed_ms(started_at))
        _safe_emit(self.signals.finished, self.request_id, summary)


class ArchiveInspectionTask(QRunnable):
    def __init__(self, request_id: int, paths: AppPaths, *, repair: bool = False):
        super().__init__()
        self.request_id = request_id
        self.paths = paths
        self.repair = repair
    ...
    def run(self) -> None:
        ...
        try:
            if self.repair:
                refresh_archive_workbook(self.paths.archive_path)
            inspection = inspect_archive(self.paths.archive_path)
        except Exception as exc:
            logger.exception("Could not inspect archive")
            _safe_emit(self.signals.failed, self.request_id, "archive_check", str(exc))
            return
        perf_log("archive.explicit_check", repair=self.repair, elapsed_ms=elapsed_ms(started_at))
        _safe_emit(self.signals.finished, self.request_id, inspection)
...
class MainWindow(QMainWindow):
    def __init__(
        ...
        self._priority_request_id = 0
        self._handoff_request_id = 0
        self._predictive_request_id = 0
        self._details_request_id = 0
        self._issue_detail_request_id = 0
        self._intake_suggestion_request_id = 0
        self._data_health_request_id = 0
        self._archive_inspection_request_id = 0
        ...
        self.setWindowTitle("BeeLine Future Issue Tracker")
        ...
        self.priority_board_page = PriorityBoardPage(theme_manager, paths)
        ...
        self.shift_handoff_page = ShiftHandoffPage(theme_manager, paths)
        ...
        self.stack.addWidget(self.priority_board_page)
        ...
        self.stack.addWidget(self.shift_handoff_page)
        ...
        self.dashboard.priority_board_requested.connect(self.show_priority_board)
        self.dashboard.shift_handoff_requested.connect(self.show_shift_handoff)
        ...
        self.priority_board_page.back_requested.connect(self.show_dashboard)
        self.priority_board_page.refresh_requested.connect(self.show_priority_board)
        self.priority_board_page.machine_requested.connect(
            lambda machine_number: self.show_machine(machine_number, return_context="priority_board")
        )
        self.priority_board_page.issue_open_requested.connect(
            lambda mode, issue_id: self.show_issue_detail(mode, issue_id, return_context="priority_board")
        )
        ...
        self.log_issue_page.suggestions_requested.connect(self.request_intake_suggestions)
        self.log_issue_page.suggestion_open_requested.connect(
            lambda issue_id: self.show_resolved_issue_detail(issue_id, return_context="log_issue")
        )
        ...
        self.settings_page.data_health_requested.connect(self.refresh_data_health)
        self.settings_page.archive_check_requested.connect(self.check_excel_archive)
        self.settings_page.archive_repair_requested.connect(self.repair_excel_archive)
        self.settings_page.machine_config_save_requested.connect(self.save_machine_config)
        self.shift_handoff_page.back_requested.connect(self.show_dashboard)
        self.shift_handoff_page.refresh_requested.connect(self.show_shift_handoff)
        self.shift_handoff_page.machine_requested.connect(
            lambda machine_number: self.show_machine(machine_number, return_context="shift_handoff")
        )
        self.shift_handoff_page.issue_open_requested.connect(
            lambda mode, issue_id: self.show_issue_detail(mode, issue_id, return_context="shift_handoff")
        )
    ...
    def return_from_machine(self) -> None:
        ...
        if self._machine_return_context == "open_issues":
            ...
            return
        if self._machine_return_context == "priority_board":
            self.show_priority_board()
            return
        if self._machine_return_context == "shift_handoff":
            self.show_shift_handoff()
            return
    ...
    def show_priority_board(self) -> None:
        started_at = perf_now()
        self._priority_request_id += 1
        request_id = self._priority_request_id
        self.priority_board_page.show_loading()
        self._current_detail = None
        self.stack.set_current_widget_animated(self.priority_board_page)
        task = PriorityBoardLoadTask(request_id, self.repository)
        task.signals.finished.connect(self._apply_priority_board_snapshot)
        task.signals.failed.connect(self._handle_page_load_failed)
        QTimer.singleShot(0, lambda request_id=request_id, task=task: self._start_priority_load_task(request_id, task))
        perf_log("navigation.show_priority_board", request_id=request_id, elapsed_ms=elapsed_ms(started_at))

    def show_shift_handoff(self) -> None:
        started_at = perf_now()
        self._handoff_request_id += 1
        request_id = self._handoff_request_id
        window_key = self.shift_handoff_page.current_window_key()
        self.shift_handoff_page.show_loading()
        self._current_detail = None
        self.stack.set_current_widget_animated(self.shift_handoff_page)
        task = ShiftHandoffLoadTask(request_id, self.repository, window_key)
        task.signals.finished.connect(self._apply_shift_handoff_snapshot)
        task.signals.failed.connect(self._handle_page_load_failed)
        QTimer.singleShot(0, lambda request_id=request_id, task=task: self._start_handoff_load_task(request_id, task))
        perf_log(
            "navigation.show_shift_handoff",
            request_id=request_id,
            window=window_key,
            elapsed_ms=elapsed_ms(started_at),
        )
    ...
    def _return_to_context(self, return_context: str) -> None:
        if return_context == "open_issues":
            self.show_open_issues()
            return
        if return_context == "priority_board":
            self.show_priority_board()
            return
        if return_context == "shift_handoff":
            self.show_shift_handoff()
            return
        if return_context == "log_issue" and self.log_issue_page.machine_number_value:
            self.stack.set_current_widget_animated(self.log_issue_page)
            return
    ...
    def request_intake_suggestions(self, machine_number: str, query: str) -> None:
        self._intake_suggestion_request_id += 1
        request_id = self._intake_suggestion_request_id
        task = IntakeSuggestionsTask(request_id, self.repository, machine_number, query)
        task.signals.finished.connect(self._apply_intake_suggestions)
        task.signals.failed.connect(self._handle_page_load_failed)
        QTimer.singleShot(
            0,
            lambda request_id=request_id, task=task: self._start_intake_suggestions_task(request_id, task),
        )

    def _apply_intake_suggestions(self, request_id: int, payload) -> None:
        if request_id != self._intake_suggestion_request_id:
            perf_log("smart_intake.snapshot_ignored", request_id=request_id)
            return
        if self.stack.currentWidget() != self.log_issue_page:
            return
        self.log_issue_page.apply_suggestions(
            payload.get("query", ""),
            payload.get("suggestions", []),
        )

    def refresh_data_health(self) -> None:
        if not can_open_settings(self.current_role):
            return
        self._data_health_request_id += 1
        request_id = self._data_health_request_id
        task = DataHealthTask(request_id, self.repository, self.paths)
        task.signals.finished.connect(self._apply_data_health)
        task.signals.failed.connect(self._handle_page_load_failed)
        self.statusBar().showMessage("Refreshing data health...", 3000)
        QTimer.singleShot(0, lambda request_id=request_id, task=task: self._start_data_health_task(request_id, task))

    def _apply_data_health(self, request_id: int, summary) -> None:
        if request_id != self._data_health_request_id:
            return
        self.settings_page.set_data_health(summary)
        self.statusBar().showMessage("Data health refreshed.", 3000)

    def check_excel_archive(self) -> None:
        self._start_archive_explicit_check(repair=False)

    def repair_excel_archive(self) -> None:
        self._start_archive_explicit_check(repair=True)

    def _start_archive_explicit_check(self, *, repair: bool) -> None:
        if not can_open_settings(self.current_role):
            return
        self._archive_inspection_request_id += 1
        request_id = self._archive_inspection_request_id
        action = "Repairing" if repair else "Checking"
        self.settings_page.set_archive_check_status(f"{action} Excel archive on demand...")
        task = ArchiveInspectionTask(request_id, self.paths, repair=repair)
        task.signals.finished.connect(self._apply_archive_inspection)
        task.signals.failed.connect(self._handle_archive_inspection_failed)
        QTimer.singleShot(
            0,
            lambda request_id=request_id, task=task: self._start_archive_inspection_task(request_id, task),
        )

    def _apply_archive_inspection(self, request_id: int, inspection) -> None:
        if request_id != self._archive_inspection_request_id:
            return
        if inspection.error:
            text = f"Excel archive check failed: {inspection.error}"
        else:
            text = (
                f"Excel archive exists: {'yes' if inspection.exists else 'no'} | "
                f"rows: {inspection.row_count} | "
                f"grouped view: {'yes' if inspection.grouped_sheet_exists else 'no'} | "
                f"latest cache row: {inspection.latest_cache_id or 'none'}"
            )
        self.settings_page.set_archive_check_status(text)
        self.statusBar().showMessage("Excel archive check finished.", 4000)

    def _handle_archive_inspection_failed(self, request_id: int, context: str, message: str) -> None:
        if request_id != self._archive_inspection_request_id:
            return
        self.settings_page.set_archive_check_status(f"Excel archive check failed: {message}")
        self._handle_page_load_failed(request_id, context, message)

    def save_machine_config(self, values: dict, original_machine_number: str, deactivate: bool) -> None:
        if not can_open_settings(self.current_role):
            QMessageBox.warning(self, "Admin required", "Machine config is available after Admin login.")
            return
        try:
            backup_path = save_machine_config_file(
                self.paths.runtime_config_path,
                self.paths.backups_dir,
                values,
                original_machine_number=original_machine_number,
                deactivate=deactivate,
            )
        except Exception as exc:
            QMessageBox.critical(self, "Could not save machine config", str(exc))
            return
        action = "deactivated" if deactivate else "saved"
        self.settings_page.set_machine_config_status(
            f"Machine config {action}. Backup: {backup_path}. Restart BeeLine Future to apply changes."
        )
        self.statusBar().showMessage("Machine config saved. Restart BeeLine Future to apply changes.", 6000)
    ...
    def _start_priority_load_task(self, request_id: int, task: QRunnable) -> None:
        if not self._closing and request_id == self._priority_request_id:
            self.page_load_pool.start(task)

    def _start_handoff_load_task(self, request_id: int, task: QRunnable) -> None:
        if not self._closing and request_id == self._handoff_request_id:
            self.page_load_pool.start(task)
    ...
    def _start_intake_suggestions_task(self, request_id: int, task: QRunnable) -> None:
        if not self._closing and request_id == self._intake_suggestion_request_id:
            self.page_load_pool.start(task)

    def _start_data_health_task(self, request_id: int, task: QRunnable) -> None:
        if not self._closing and request_id == self._data_health_request_id:
            self.page_load_pool.start(task)

    def _start_archive_inspection_task(self, request_id: int, task: QRunnable) -> None:
        if not self._closing and request_id == self._archive_inspection_request_id:
            self.thread_pool.start(task)
    ...
    def _apply_priority_board_snapshot(self, request_id: int, snapshot) -> None:
        if request_id != self._priority_request_id or self.stack.currentWidget() != self.priority_board_page:
            perf_log("priority_board.snapshot_ignored", request_id=request_id)
            return
        started_at = perf_now()
        self.priority_board_page.apply_snapshot(snapshot)
        perf_log("priority_board.apply_snapshot", request_id=request_id, elapsed_ms=elapsed_ms(started_at))

    def _apply_shift_handoff_snapshot(self, request_id: int, snapshot) -> None:
        if request_id != self._handoff_request_id or self.stack.currentWidget() != self.shift_handoff_page:
            perf_log("shift_handoff.snapshot_ignored", request_id=request_id)
            return
        started_at = perf_now()
        self.shift_handoff_page.apply_snapshot(snapshot)
        perf_log("shift_handoff.apply_snapshot", request_id=request_id, elapsed_ms=elapsed_ms(started_at))
    ...
    def refresh_current_views(self) -> None:
        ...
        if current == self.open_issues_page:
            self.show_open_issues()
        if current == self.priority_board_page:
            self.show_priority_board()
        if current == self.shift_handoff_page:
            self.show_shift_handoff()
    ...
    def show_settings(self) -> None:
        ...
        QTimer.singleShot(0, self.refresh_data_health)
    ...
    def _ready_status_text(self) -> str:
        pi_note = " | Pi mode" if os.environ.get("BEELINE_PI_MODE", "").strip() else ""
        return f"Ready | BeeLine Future | SQLite live cache active{pi_note}"
    ...
    def closeEvent(self, event) -> None:
        ...
        self.log_issue_page._suggestion_timer.stop()
