...
class OpenIssuesPage(HoneycombBackground):
    ...
    def _configure_table(self) -> None:
        ...
        widths = (150, 190, 210, 126, 260, 120, 145, 82, 128, 110, 240)
    ...
    def _populate_table(self, issues: list[Issue]) -> None:
        ...
        try:
            ...
            for row, issue in enumerate(issues):
                ...
                age = issue_age(issue)
                self.table.setItem(row, 7, self._item(age.label))
                self.table.setItem(row, 8, self._item(age.state))
                self.table.setItem(row, 9, self._item(issue.category or "-", issue.category or None))
                self.table.setCellWidget(row, 10, self._actions_widget(issue))
