...
CARD_COLOR_STYLE_BRIGHT = "bright"
CARD_COLOR_STYLE_LEGACY = "legacy"
DEFAULT_CARD_COLOR_STYLE = CARD_COLOR_STYLE_BRIGHT
CARD_COLOR_STYLE_LABELS = {
    CARD_COLOR_STYLE_BRIGHT: "Card Color Style Bright",
    CARD_COLOR_STYLE_LEGACY: "Card Color Style Legacy",
}
...
THEMES: dict[str, ThemeTokens] = {
    DARK_THEME: ThemeTokens(
        ...
        input_border="#4a4d4f",
        status_line_down="#FF073A",
        status_line_down_text="#fff7f0",
        status_non_critical="#FFF700",
        status_non_critical_text="#1b1710",
        status_no_issues="#39FF14",
...
class ThemeManager(QObject):
    ...
    CARD_STYLE_SETTINGS_KEY = "ui/card_color_style"
    ...
    def __init__(self, settings: QSettings | None = None, parent=None):
        ...
        stored_card_style = str(self.settings.value(self.CARD_STYLE_SETTINGS_KEY, DEFAULT_CARD_COLOR_STYLE))
        self._card_color_style = (
            stored_card_style
            if stored_card_style in CARD_COLOR_STYLE_LABELS
            else DEFAULT_CARD_COLOR_STYLE
        )
    ...
    @property
    def current_card_color_style(self) -> str:
        return self._card_color_style

    @property
    def current_card_color_style_label(self) -> str:
        return CARD_COLOR_STYLE_LABELS[self._card_color_style]
    ...
    def set_card_color_style(self, card_color_style: str) -> None:
        if card_color_style not in CARD_COLOR_STYLE_LABELS:
            raise ValueError(f"Unknown BeeLine card color style: {card_color_style}")
        if card_color_style == self._card_color_style:
            return
        self._card_color_style = card_color_style
        self.settings.setValue(self.CARD_STYLE_SETTINGS_KEY, card_color_style)
        self.settings.sync()
        self.theme_changed.emit(card_color_style)

    def build_stylesheet(self) -> str:
        return build_stylesheet(self.current_theme, self.current_card_color_style)
...
def build_stylesheet(
    theme: ThemeTokens | str | None = None,
    card_color_style: str = DEFAULT_CARD_COLOR_STYLE,
) -> str:
    tokens = theme_from_name(theme) if isinstance(theme, str) or theme is None else theme
    machine_card_styles = _machine_card_styles(tokens, card_color_style)
    machine_card_badge_styles = _machine_card_badge_styles(card_color_style)
    return f"""
    QMainWindow, QWidget {{
...
def _machine_card_styles(tokens: ThemeTokens, card_color_style: str) -> str:
    if card_color_style == CARD_COLOR_STYLE_LEGACY:
        return f"""
    ...
    bright_dark_text = "#101410"
    bright_light_text = "#fff8f8"
    return f"""
    QFrame#machineCard {{
    ...
    QFrame#machineCard[statusState="unknown"] {{
        background-color: {tokens.status_unknown};
        border-color: {tokens.status_unknown};
    }}

    QFrame#machineCard[statusState="line_down"] QLabel {{
        color: {bright_light_text};
    }}

    QFrame#machineCard[statusState="non_critical"] QLabel,
    QFrame#machineCard[statusState="no_issues"] QLabel,
    QFrame#machineCard[statusState="unknown"] QLabel {{
        color: {bright_dark_text};
    }}

    """


def _machine_card_badge_styles(card_color_style: str) -> str:
    if card_color_style != CARD_COLOR_STYLE_BRIGHT:
        return ""
    return """
    QFrame#machineCard[statusState="line_down"] QLabel#statusBadge {
        background-color: #fff8f8;
        color: #B00024;
    }

    QFrame#machineCard[statusState="non_critical"] QLabel#statusBadge {
        background-color: #101410;
        color: #FFF700;
    }

    QFrame#machineCard[statusState="no_issues"] QLabel#statusBadge {
        background-color: #101410;
        color: #39FF14;
    }

    QFrame#machineCard[statusState="unknown"] QLabel#statusBadge {
        background-color: #101410;
        color: #f4f7f7;
    }
    """
