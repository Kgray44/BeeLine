...
from beeline_issue_tracker.future_features import KnownFix
...
def load_machine_details_snapshot(
    ...
    call_started = perf_now()
    known_fixes = repository.list_known_fixes(summary.machine_number, limit=10)
    perf_log("details.known_fixes", machine=summary.machine_number, count=len(known_fixes), elapsed_ms=elapsed_ms(call_started))
...
class MachineDetailsPage(HoneycombBackground):
    ...
    def apply_snapshot(self, snapshot: MachineDetailsSnapshot) -> None:
        ...
        self._add_known_fixes(snapshot.known_fixes)
    ...
    def _add_known_fixes(self, known_fixes: list[KnownFix]) -> None:
        panel = self._panel("Known Fixes", "known_fixes")
        layout = panel.layout()
        assert layout is not None
        if not known_fixes:
            layout.addWidget(self._muted("No repeated resolved fixes have been found for this machine yet."))
            self.content_layout.addWidget(panel)
            return

        for fix in known_fixes[:10]:
            row = QFrame()
            ...
            text = QLabel(
                f"{fix.pattern}\n"
                f"{fix.category} | seen {fix.times_seen} | last used {format_timestamp(fix.last_used)}\n"
                f"{fix.solution_preview}"
            )
            ...
            open_button.clicked.connect(
                lambda _checked=False, issue_id=fix.related_issue_id: self.issue_detail_requested.emit(issue_id, "resolved")
            )
