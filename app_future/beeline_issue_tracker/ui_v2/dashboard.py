...
class HiveDashboardPage(HoneycombBackground):
    ...
    priority_board_requested = Signal()
    shift_handoff_requested = Signal()
    ...
    def __init__(self, repository: IssueRepository, theme_manager: ThemeManager, paths: AppPaths, parent=None):
        ...
        header = QHBoxLayout()
        header.addWidget(BrandHeader("BeeLine Future", "Pi-first Hive Dashboard", paths.logo_path(), theme_manager))
        header.addStretch(1)
        priority_board = QPushButton("Priority Board")
        priority_board.setObjectName("primaryButton")
        priority_board.clicked.connect(self.priority_board_requested.emit)
        header.addWidget(priority_board)
        shift_handoff = QPushButton("Shift Handoff")
        shift_handoff.setObjectName("secondaryButton")
        shift_handoff.clicked.connect(self.shift_handoff_requested.emit)
        header.addWidget(shift_handoff)
