...
class IssueListView(QFrame):
    ...
    def _configure_columns(self) -> None:
        ...
        if self.include_resolved_fields:
            ...
            widths = (150, 190, 126, 310, 120, 146, 86, 128, 120, 210)
            stretch_columns = (3,)
    ...
    def _populate_active_row(self, row: int, issue: Issue) -> None:
        ...
        age = issue_age(issue)
        self.table.setItem(row, 6, self._item(age.label))
        self.table.setItem(row, 7, self._item(age.state))
        ...
        self.table.setCellWidget(row, 9, self._action_widget(view_button, resolve_button))
...
class IssueCard(QFrame):
    ...
    def __init__(self, issue: Issue, parent=None):
        ...
        meta_parts = [
            ...
            f"Machine {issue.machine_number}",
            issue.severity,
            age.label,
            age.state,
            f"Logged by {issue.logged_by}",
            issue.created_at,
        ]
        if issue.category:
