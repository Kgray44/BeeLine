...
APP_NAME = "BeeLine Future Issue Tracker"
...
class MachineConfig:
    ...
    def as_database_row(self) -> tuple[str, str, str, str, str, int, str, str, str, str, str, str, int]:
        return (
            self.machine_number,
            self.name,
            self.area,
...
class RuntimeConfig:
    ...
    def machine_rows(self) -> tuple[tuple[str, str, str, str, str, int, str, str, str, str, str, str, int], ...]:
        ...
        try:
            machines.append(
                MachineConfig(
                    ...
                    is_active=_bool_setting(machine, "is_active", True),
                )
            )
...
def _load_special_effects(raw_special: object) -> SpecialEffectsConfig:
    ...
    pi_mode = os.environ.get("BEELINE_PI_MODE", "").strip().casefold() in {"1", "true", "yes", "on"}

    return SpecialEffectsConfig(
        enabled=False if pi_mode else _bool_setting(raw_special, "enabled", defaults.enabled),
        ...
        enable_static=False if pi_mode else _bool_setting(raw_special, "enable_static", defaults.enable_static),
        enable_glitch=False if pi_mode else _bool_setting(raw_special, "enable_glitch", defaults.enable_glitch),
        enable_droop_drip=False if pi_mode else _bool_setting(raw_special, "enable_droop_drip", defaults.enable_droop_drip),
        enable_smear=False if pi_mode else _bool_setting(raw_special, "enable_smear", defaults.enable_smear),
        enable_card_impulses=_bool_setting(
            raw_special,
            "enable_card_impulses",
            False if pi_mode else defaults.enable_card_impulses,
        ),
        enable_falling_drips=_bool_setting(
            raw_special,
            "enable_falling_drips",
            False if pi_mode else defaults.enable_falling_drips,
        ),
        ...
        reduced_motion=True if pi_mode else _bool_setting(raw_special, "reduced_motion", defaults.reduced_motion),
